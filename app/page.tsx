...
export default memo(function Page(){
  ...
  // error helper
  const onErr=useCallback((e:unknown)=>setError(`失败: ${e instanceof Error?e.message:String(e)}`),[]);
  ...
  const readClip=useCallback(async(hash:string)=>{
    ...
    }catch(e){onErr(e);}finally{setState(LoadingState.Idle);abortRef.current=null;}
  },[onErr]);
  ...
  const reset=()=>{setInput('');setShareUrl('');setRawUrl('');setDecrypted(null);setError(null);setState(LoadingState.Idle);setShowRaw(false);history.replaceState(null,'',location.pathname+location.search);};
