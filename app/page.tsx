          ...
          {!isReading && !decrypted && !shareUrl && !isNotFound && (
            <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col flex-1">
              ...
              <div className="flex flex-col flex-1">
                ...
                <textarea id="main-input" placeholder="在此处输入要分享的文本…" value={input} onChange={e => setInput(e.target.value)} disabled={isCreating} className={`flex-1 p-4 ${themeClasses.inputBg} placeholder-neutral-400 resize-none focus:outline-none border ${themeClasses.border} rounded-md ${isCreating ? 'opacity-50' : ''} transition-colors duration-300`} autoFocus />
              </div>
              ...
              <div className="mt-6 flex justify-end">
                <button onClick={upload} disabled={!input || isCreating} className={`px-6 py-2 ${themeClasses.btnPrimary} ${(!input || isCreating) ? 'opacity-50' : ''} transition-colors duration-300`}>
